...
api.interceptors.request.use(function (config) {
  ...
  if (token) {
    // config.headers가 있는 경우에는 그대로 놔두고, 없는 경우에는 빈 객체를 넣어준다.
...
async function drawPostList() {
  ...
  // 3. 필요한 데이터 불러오기
  const { data: postList } = await api.get('/posts?_expand=user')//{data:}??
...
async function drawPostDetail(postId) {
  ...
  // 같은 이름의 쿼리 스트링을 여러개 쓰고 싶은 경우 URLSearchParams를 사용해야 한다.
